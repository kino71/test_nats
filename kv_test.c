...
void errHandler(natsConnection* nc, natsSubscription* sub, natsStatus s, void* closure) {
    std::ostringstream out;
    natsStatus last;
    auto lastText = nats_GetLastError(&s);
    out << "ERROR : [NATS] Server : " << getConnectedServer(nc) << "] [ConnectionStatus " << s << " = " << natsStatus_GetText(s) << "][LastError=" << last << " = " << lastText << "][closure=" << (closure ? "NOT " : "") << "NULL]"  << std::endl;
    std::cerr << out.str();
}
