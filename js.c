...
_recreateOrderedCons(void *closure)
{
    ...
    fprintf(stderr, "[%s,%d] _recreateOrderedCons(void *closure) oci->done=%d oci->max=%d)\n", __FILE__, __LINE__, oci->done, oci->max);
    ...
    if (!oci->done)
    {
	fprintf(stderr, "[%s,%d] if (!oci->done) INSIDE oci->done=%d oci->max=%d)\n", __FILE__, __LINE__, oci->done, oci->max);
        IFOK(s, natsConn_sendSubProto(nc, oci->ndlv, NULL, oci->nsid));
        if ((s == NATS_OK) && (oci->max > 0)) 
	{
  	    fprintf(stderr, "[%s,%d] f ((s == NATS_OK) && (oci->max > 0)) INSIDE oci->done=%d oci->max=%d)\n", __FILE__, __LINE__, oci->done, oci->max);
    ...
    {
	fprintf(stderr, "[%s,%d] if (!oci->done && (s == NATS_OK) INSIDE oci->done=%d oci->max=%d)\n", __FILE__, __LINE__, oci->done, oci->max);
        ...
        if (s == NATS_OK)
        {
	    fprintf(stderr, "[%s,%d] js_AddConsumer(&ci, jsi->js, jsi->stream, cc, NULL, NULL) stream=%s consumer=%s ocCfg.name=%s ocCfg.durable=%s ocCfg.description=%s\n", 
		    __FILE__, __LINE__,
		   jsi->stream,
		   jsi->consumer,
		   jsi->ocCfg->Name,
		   jsi->ocCfg->Durable,
		   jsi->ocCfg->Description
	    );
            s = js_AddConsumer(&ci, jsi->js, jsi->stream, cc, NULL, NULL);
            if (s == NATS_OK)
            {
	    	fprintf(stderr, "[%s,%d] if (s == NATS_OK)\n", __FILE__, __LINE__);
                natsSub_Lock(sub);
                NATS_FREE(jsi->consumer);
                jsi->consumer = NULL;
	    	...
	    	fprintf(stderr, "[%s,%d] if (s == NATS_OK) else %d %s\n", __FILE__, __LINE__, s, natsStatus_GetText(s));
        }
    }
    if (s != NATS_OK)
    {
